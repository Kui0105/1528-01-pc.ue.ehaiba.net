<template>
    <div class="xl:flex">
        ...
        <el-card class="!border-none flex-1 mb-4" shadow="never">
            <template #header><span class="font-medium">关于我们</span></template>
            <el-form :model="formData" label-width="80px"><el-form-item label="页面标题"><el-input v-model="formData.about_title" /></el-form-item></el-form>
            <editor v-model="formData.about_content" height="500" />
        </el-card>
    </div>
...
const formData = ref<formDataObj>({
    ...
    service_content: '',
    privacy_title: '',
    privacy_content: '', about_title: '关于我们', about_content: '<p>欢迎了解我们。</p>'
})
const protocolGet = async () => {
    formData.value = { ...formData.value, ...(await getProtocol()) }
}
